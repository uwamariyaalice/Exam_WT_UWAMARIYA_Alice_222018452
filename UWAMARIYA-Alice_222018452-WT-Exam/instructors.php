...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
    ...
    $stmt->bind_param("iis", $InstructorID, $UserID, $Bio);
...
<table border="1">
    ...
    <?php
    include('database_connection.php');
    // Retrieve all records from the instructors table
    $stmt = $connection->prepare("SELECT * FROM instructors");
    $stmt->execute();
    ...
    if ($result->num_rows > 0) { 
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                ...
                <td>" . $row["UserID"] . "</td>
                <td>" . $row["Bio"] . "</td>
                <td><a href='delete_instructors.php?InstructorID=$instructorID'>Delete</a></td> 
                <td><a href='update_instructors.php?InstructorID=$instructorID'>Update</a></td> 
            </tr>";
        }
    } else {
